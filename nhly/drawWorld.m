...
cityPath = importdata('data/world/data_path.txt');
s = size(cityPath);
...
%         p =  cat(2, zeros(2, 4 - pSize(1,2)), p);
    ...
    dlmwrite('data/world/timeTable.txt',t,'delimiter','\t','newline','pc','-append');
    
    drawPathByPoly(p(1,:), p(2,:));
    drawPoints(x, y);

    p =  cat(2,[citySrc;cityDst], p);
    dlmwrite('data/world/polyTable.txt',p,'delimiter','\t','newline','pc','-append');
end
