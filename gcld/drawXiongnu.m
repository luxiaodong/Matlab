function [ ] = drawXiongnu()
%CITY_MAIN Summary of this function goes here
%   Detailed explanation goes here

cityPosition = load('data/xiongnu/data_city.txt');

cityPath = importdata('data/xiongnu/data_path.txt');
s = size(cityPath);
    ...
    for j=3:s(1,2)
        ...
        if isnan(value)
            ...
            if mod(j,2) == 1
                ...
                y = cat(2,y, 1800 - value);
            end
        end
    end
    
    cityDst = row(1,2);
    cityDstIndex = cityDst - 9300;
...
%         p =  cat(2, zeros(2, 4 - pSize(1,2)), p);
    ...
    dlmwrite('data/xiongnu/timeTable.txt',t,'delimiter','\t','newline','pc','-append');
    
    drawPathByPoly(p(1,:), p(2,:));
    drawPoints(x, y);

    p =  cat(2,[citySrc;cityDst], p);
    dlmwrite('data/xiongnu/polyTable.txt',p,'delimiter','\t','newline','pc','-append');
end
