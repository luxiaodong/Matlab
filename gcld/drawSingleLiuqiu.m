function [] = drawSingleLiuqiu(index, width, height)
    
cityPosition = load( sprintf('data/liuqiu/city_pos_%d.txt', index) );

cityPath = importdata( sprintf('data/liuqiu/city_path_%d.txt', index) );
s = size(cityPath);
...
%         p =  cat(2, zeros(2, 4 - pSize(1,2)), p);
    ...
    dlmwrite( sprintf('data/liuqiu/timeTable_%d.txt',index),t,'delimiter','\t','newline','pc','-append');
    
    drawPathByPoly(p(1,:), p(2,:));
    drawPoints(x, y);

    p =  cat(2,[row(1,1);row(1,2)], p);
    dlmwrite( sprintf('data/liuqiu/polyTable_%d.txt',index),p,'delimiter','\t','newline','pc','-append');
end
